...
export default function App() {
  return (
    <div style={{ margin: 0, padding: 0, background: C.bg, overflowX: "hidden" }}>
      ...
      {/* ══════ DEVELOPERS ══════ */}
      <section id="developers" className="section-pad" style={{ background: "#fff", padding: "120px 48px", borderTop: "1px solid rgba(0,0,0,0.05)" }}>
        <div style={{ maxWidth: 1060, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 72 }}>
              <Label color={C.accent}>Developers</Label>
              <h2 className="section-h2" style={{ fontSize: 46, fontWeight: 800, color: C.text, fontFamily: F1, letterSpacing: -1.8, margin: "0 0 16px" }}>Build with HTGC</h2>
              <p style={{ fontSize: 17, color: C.sub, fontFamily: F2, maxWidth: 520, margin: "0 auto" }}>Integrate the Haitian gourde into your app in minutes. Native SDKs for all three chains.</p>
            </div>
          </Reveal>

          {/* Chain tabs / cards */}
          <div className="three-col" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, marginBottom: 56 }}>
            {[
              {
                name: "Solana", color: "#D12229", grad: "linear-gradient(135deg,#D12229,#E63946)",
                contract: "HTGCxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
                install: "npm install @solana/web3.js @solana/spl-token",
                snippet: `import { getAssociatedTokenAddress } from "@solana/spl-token";

const HTGC_MINT = new PublicKey("HTGCxxxx...");
const ata = await getAssociatedTokenAddress(
  HTGC_MINT,
  wallet.publicKey
);`,
                links: [
                  { l: "Token-2022 Docs", h: "https://spl.solana.com/token-2022" },
                  { l: "Jupiter Integration", h: "https://docs.jup.ag" },
                  { l: "Solscan Explorer", h: "https://solscan.io" },
                ],
              },
              {
                name: "Stellar", color: "#00209F", grad: "linear-gradient(135deg,#001166,#00209F)",
                contract: "HTGC-GXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
                install: "npm install @stellar/stellar-sdk",
                snippet: `import { Asset, Horizon } from "@stellar/stellar-sdk";

const HTGC = new Asset(
  "HTGC",
...
const server = new Horizon.Server(
  "https://horizon.stellar.org"
                ...
                links: [
                  { l: "Stellar Docs", h: "https://developers.stellar.org" },
                  { l: "SEP-24 Guide", h: "https://developers.stellar.org/docs/anchoring-assets" },
                  { l: "Stellar Expert", h: "https://stellar.expert" },
                ],
              },
              {
                name: "Base", color: "#0052FF", grad: "linear-gradient(135deg,#0040CC,#0052FF)",
                contract: "0xHTGC000000000000000000000000000000000",
                install: "npm install viem @wagmi/core",
                snippet: `import { createPublicClient, http } from "viem";
import { base } from "viem/chains";

const HTGC_ADDRESS = "0xHTGC0000...";
const client = createPublicClient({
  chain: base,
  transport: http(),
});`,
                links: [
                  { l: "Base Docs", h: "https://docs.base.org" },
                  { l: "Basescan Explorer", h: "https://basescan.org" },
                  { l: "Uniswap on Base", h: "https://app.uniswap.org" },
                ],
              },
            ].map((chain, i) => (
              <Reveal key={i} delay={i * 0.1}>
                <div style={{ background: "#F8F8FA", borderRadius: 24, overflow: "hidden", border: "1px solid rgba(0,0,0,0.07)" }}>
                  {/* Header */}
                  <div style={{ background: chain.grad, padding: "24px 28px", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: "#fff", fontFamily: F1 }}>{chain.name[0]}</div>
                    <div>
                      <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: F1 }}>{chain.name}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", fontFamily: F2 }}>HTGC Token</div>
                    </div>
                    <img src={tokenImg} alt="" style={{ width: 28, height: 28, objectFit: "contain", marginLeft: "auto", opacity: 0.9 }} />
                  </div>
                  <div style={{ padding: "24px 28px" }}>
                    {/* Contract */}
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: C.gray, fontFamily: F1, letterSpacing: 2, marginBottom: 6 }}>CONTRACT ADDRESS</div>
                      <div style={{ fontSize: 11, color: C.sub, fontFamily: "monospace", background: "#fff", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(0,0,0,0.07)", wordBreak: "break-all" }}>{chain.contract}</div>
                    </div>
                    {/* Install */}
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: C.gray, fontFamily: F1, letterSpacing: 2, marginBottom: 6 }}>INSTALL</div>
                      <div style={{ fontSize: 11.5, color: chain.color, fontFamily: "monospace", background: "#fff", padding: "10px 14px", borderRadius: 8, border: `1px solid ${chain.color}22` }}>{chain.install}</div>
                    </div>
                    {/* Code snippet */}
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: C.gray, fontFamily: F1, letterSpacing: 2, marginBottom: 6 }}>QUICK START</div>
                      <pre style={{ fontSize: 10.5, color: "#1a1a2e", fontFamily: "monospace", background: "#fff", padding: "14px 16px", borderRadius: 8, border: "1px solid rgba(0,0,0,0.07)", overflowX: "auto", margin: 0, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{chain.snippet}</pre>
                    </div>
                    {/* Links */}
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {chain.links.map((lk, j) => (
                        <a key={j} href={lk.h} target="_blank" rel="noopener noreferrer"
                          style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13, color: chain.color, fontFamily: F2, fontWeight: 500, textDecoration: "none", padding: "8px 12px", background: `${chain.color}08`, borderRadius: 8, transition: "background 0.2s" }}
                          onMouseEnter={e => (e.currentTarget.style.background = `${chain.color}16`)}
                          onMouseLeave={e => (e.currentTarget.style.background = `${chain.color}08`)}>
                          {lk.l} <span style={{ fontSize: 11 }}>↗</span>
                        </a>
                      ))}
                    </div>
                  </div>
          ...
          {/* API overview row */}
          <Reveal delay={0.2}>
            <div style={{ background: "#F8F8FA", borderRadius: 24, padding: "40px 48px", border: "1px solid rgba(0,0,0,0.07)", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 32 }} className="three-col">
              {[
                { icon: "⚡", t: "REST API", d: "Unified API to mint, burn, and query HTGC balances across all three chains from a single endpoint.", cta: "API Reference →", color: C.accent },
                { icon: "🔔", t: "Webhooks", d: "Real-time event notifications for transfers, mints, burns, and compliance flags on any chain.", cta: "Webhook Docs →", color: C.teal },
                { icon: "🛡", t: "Compliance SDK", d: "Built-in KYC/AML hooks, freeze authority, and travel rule metadata for regulated use cases.", cta: "Compliance Guide →", color: "#0052FF" },
              ].map((item, i) => (
                <div key={i}>
                  <div style={{ fontSize: 28, marginBottom: 12 }}>{item.icon}</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: C.text, fontFamily: F1, marginBottom: 8 }}>{item.t}</div>
                  <p style={{ fontSize: 13.5, color: C.sub, fontFamily: F2, lineHeight: 1.7, margin: "0 0 16px" }}>{item.d}</p>
                  <a href="#" style={{ fontSize: 13, fontWeight: 600, color: item.color, fontFamily: F1, textDecoration: "none" }}>{item.cta}</a>
                </div>
              ))}
            </div>
          </Reveal>
